...
%% is not required. acmart still prints its defaults (an ACM copyright
%% notice and a placeholder DOI), so they are switched off here.
\setcopyright{none}
\settopmatter{printacmref=false}
\renewcommand\footnotetextcopyrightpermission[1]{}
%% acmart's own "not an ACM publication" switch: also drops the
%% "Manuscript submitted to ACM" footer. Layout is unchanged.
\makeatletter
\@ACM@nonacmtrue
\makeatother
